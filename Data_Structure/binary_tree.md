...
public:
    ...
    TreeNode* DFS(const vector<int>& preorder, const vector<int>& inorder, int pl, int pr, int il, int ir)
    {
        ...
        int root_pos = inorder_index[preorder[pl]];

        // left_tree_size = root_pos-il
        // right_tree_size = ir-root_pos
        // left_tree_preorder_start = pl+1
        // left_tree_preorder_end = pl+root_pos-il
        // left_tree_inorder_start = il;
        // left_tree_inorder_end = root_pos-1;
        // right_tree_preorder_start = pl+root_pos-il+1
        // right_tree_preorder_end = pr
        // right_tree_inorder_start = root_pos+1;
        // right_tree_inorder_end = ir;

        TreeNode* root = new TreeNode(preorder[pl]);

        root->left = DFS(preorder, inorder, pl+1, pl+root_pos-il, il, root_pos-1);
        root->right = DFS(preorder, inorder, pl+root_pos-il+1, pr, root_pos+1, ir);
